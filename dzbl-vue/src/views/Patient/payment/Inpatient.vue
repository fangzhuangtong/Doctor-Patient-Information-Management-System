  ...
  <div class="payment">
    <el-card class="payment-card">
      ...
      <el-table :data="tableData" style="width: 100%" v-loading="loading">
        ...
        <el-table-column prop="relatedId" label="住院ID" width="100" />
...
const getList = async () => {
  ...
  try {
    ...
    if (res.code === 1) {
      // 过滤数据，只保留住院类型的订单
      const inpatientOrders = res.data.list.filter(order => order.orderType === '住院')
      tableData.value = inpatientOrders
      // 更新总数为过滤后的数量
      total.value = inpatientOrders.length
    }
  } catch (error) {
