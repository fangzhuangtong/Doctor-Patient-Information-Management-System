      ...
      <el-table :data="tableData" style="width: 100%" v-loading="loading">
        ...
        <el-table-column prop="reportType" label="报告类型" width="100">
          <template #default="scope">
            <el-tag :type="scope.row.reportType === '检查' ? 'success' : 'warning'">
              {{ scope.row.reportType }}
            </el-tag>
          </template>
        </el-table-column>
        <el-table-column prop="conclusion" label="检查结论" min-width="200" show-overflow-tooltip />
        <el-table-column prop="advice" label="医生建议" min-width="200" show-overflow-tooltip />
...
const getList = async () => {
  loading.value = true
  try {
    const res = await request.get('/exam-reports', {
      params: {
        ...queryForm.value,
...
const rules = {
  ...
  advice: [{ required: true, message: '请输入医生建议', trigger: 'blur' }]
}
...
const handleDelete = (row) => {
  ElMessageBox.confirm('确认删除该报告吗？', '提示', {
    ...
    try {
      await request.delete(`/exam-reports/${row.reportId}`)
    ...
    if (valid) {
      try {
        if (dialogType.value === 'add') {
          await request.post('/exam-reports', form.value)
          ElMessage.success('添加成功')
        } else {
          const { reportId } = form.value
          await request.put(`/exam-reports/${reportId}`, {
...
const handleViewFiles = async (row) => {
  ...
  try {
    const res = await request.get('/medical-files', {
      params: {
        userId: row.userId,
        moduleType: row.reportType === '检查' ? '检查报告' : '检验报告',
