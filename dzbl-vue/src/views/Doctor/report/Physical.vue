  ...
  <div class="report">
    <el-card class="report-card">
      ...
      <el-table :data="tableData" style="width: 100%" v-loading="loading">
        ...
        <el-table-column label="体检结果" min-width="300">
          <template #default="scope">
            <div v-for="(value, key) in scope.row.results" :key="key" class="result-item">
    ...
    >
      <el-form :model="form" :rules="rules" ref="formRef" label-width="100px">
        ...
        <el-form-item label="体检ID" prop="examId" v-if="dialogType === 'add'">
          <el-input v-model.number="form.examId" type="number" />
        </el-form-item>
        <el-form-item label="血压" prop="results.bloodPressure">
          <el-input v-model="form.results.bloodPressure" placeholder="例如: 120/80" />
        </el-form-item>
        <el-form-item label="胆固醇" prop="results.cholesterol">
          <el-input v-model="form.results.cholesterol" placeholder="例如: 正常" />
        </el-form-item>
...
const getList = async () => {
  loading.value = true
  try {
    const res = await request.get('/physical-exam-reports', {
      params: {
        ...queryForm.value,
...
const rules = {
  ...
  examId: [{ required: true, message: '请输入体检ID', trigger: 'blur' }],
  'results.bloodPressure': [{ required: true, message: '请输入血压值', trigger: 'blur' }],
  'results.cholesterol': [{ required: true, message: '请输入胆固醇值', trigger: 'blur' }],
  suggestions: [{ required: true, message: '请输入体检建议', trigger: 'blur' }]
}
...
const handleDelete = (row) => {
  ElMessageBox.confirm('确认删除该报告吗？', '提示', {
    ...
    try {
      await request.delete(`/physical-exam-reports/${row.reportId}`)
    ...
    if (valid) {
      try {
        if (dialogType.value === 'add') {
          await request.post('/physical-exam-reports', form.value)
          ElMessage.success('添加成功')
        } else {
          const { reportId } = form.value
          await request.put(`/physical-exam-reports/${reportId}`, {
