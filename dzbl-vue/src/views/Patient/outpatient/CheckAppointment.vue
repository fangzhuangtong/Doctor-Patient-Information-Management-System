      ...
      <div v-else class="doctors-list" v-loading="scheduleLoading">
        <el-table :data="scheduleData" style="width: 100%">
          ...
          <el-table-column label="操作" width="200">
            <template #default="scope">
              <el-button type="primary" size="small" @click="handleAddBooking(scope.row)">
                预约检查
              </el-button>
      ...
      <el-form :model="form" :rules="rules" ref="formRef" label-width="100px">
        ...
        <el-form-item label="检查地点" prop="location">
          <el-select v-model="form.location" placeholder="请选择">
            <el-option label="1楼检验科" value="检验科" />
            <el-option label="2楼检查科" value="检查科" />
          </el-select>
        </el-form-item>
      </el-form>
      <template #footer>
...
const rules = {
  doctorId: [
    ...
    { type: 'number', message: '医生ID必须为数字', trigger: 'blur' }
  ],
  examType: [
  ...
  location: [
    { required: true, message: '请选择检查地点', trigger: 'change' }
  ]
}
...
const handleSubmit = async () => {
  ...
  await formRef.value.validate(async (valid) => {
    if (valid) {
      try {
        const res = await request.post('/exam-bookings', {
          userId,
          doctorId: form.value.doctorId,
          examType: form.value.examType,
          scheduleTime: dayjs(form.value.scheduleTime).format('YYYY-MM-DDTHH:mm:ss'),
