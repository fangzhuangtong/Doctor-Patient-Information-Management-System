  ...
  <div class="medical-record">
    <el-card class="record-card">
      ...
      <el-table :data="tableData" style="width: 100%" v-loading="loading">
        ...
        <el-table-column prop="hospitalId" label="住院ID" width="100" />
    ...
    >
      <el-form :model="form" :rules="rules" ref="formRef" label-width="100px">
        ...
        <el-form-item label="住院ID" prop="hospitalId" v-if="dialogType === 'add'">
          <el-input v-model.number="form.hospitalId" type="number" />
        </el-form-item>
        <el-form-item label="入院诊断" prop="admissionDiagnosis">
          <el-input
            v-model="form.admissionDiagnosis"
            type="textarea"
            :rows="3"
            placeholder="请输入入院诊断"
          />
        </el-form-item>
        <el-form-item label="出院小结" prop="dischargeSummary">
          <el-input
            v-model="form.dischargeSummary"
            type="textarea"
            :rows="3"
            placeholder="请输入出院小结"
          />
...
const rules = {
  ...
  hospitalId: [{ required: true, message: '请输入住院ID', trigger: 'blur' }],
  admissionDiagnosis: [{ required: true, message: '请输入入院诊断', trigger: 'blur' }],
  dischargeSummary: [{ required: true, message: '请输入出院小结', trigger: 'blur' }]
}
...
const handleEdit = async (row) => {
  ...
  const res = await request.get(`/inpatient-records/${row.recordId}`)
  if (res.code === 1) {
    form.value = { 
      ...res.data,
...
const handleDelete = (row) => {
  ElMessageBox.confirm('确认删除该病历记录吗？', '提示', {
    type: 'warning'
  }).then(async () => {
    try {
      const res = await request.delete(`/inpatient-records/${row.recordId}`)
      if (res.code === 1) {
    ...
    if (valid) {
      try {
        const api = dialogType.value === 'add' ? 
          request.post('/inpatient-records', form.value) :
          request.put(`/inpatient-records/${form.value.recordId}`, {
            admissionDiagnosis: form.value.admissionDiagnosis,
            dischargeSummary: form.value.dischargeSummary,
