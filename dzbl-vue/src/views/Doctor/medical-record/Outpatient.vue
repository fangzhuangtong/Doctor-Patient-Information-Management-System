  ...
  <div class="medical-record">
    <el-card class="record-card">
      ...
      <el-table :data="tableData" style="width: 100%" v-loading="loading">
        ...
        <el-table-column prop="registerId" label="挂号ID" width="100" />
    ...
    >
      <el-form :model="form" :rules="rules" ref="formRef" label-width="100px">
        ...
        <el-form-item label="挂号ID" prop="registerId" v-if="dialogType === 'add'">
          <el-input v-model.number="form.registerId" type="number" />
        </el-form-item>
...
const rules = {
  ...
  registerId: [{ required: true, message: '请输入挂号ID', trigger: 'blur' }],
  chiefComplaint: [{ required: true, message: '请输入主诉', trigger: 'blur' }],
  diagnosis: [{ required: true, message: '请输入诊断', trigger: 'blur' }],
  prescription: [{ required: true, message: '请输入处方', trigger: 'blur' }]
}
...
const getList = async () => {
  loading.value = true
  try {
    const res = await request.get('/outpatient-records', {
      params: {
        ...queryForm.value,
        doctorId
      }
...
const handleEdit = async (row) => {
  ...
  const res = await request.get(`/outpatient-records/${row.recordId}`)
  if (res.code === 1) {
    form.value = { 
      ...res.data,
...
const handleDelete = (row) => {
  ElMessageBox.confirm('确认删除该病历记录吗？', '提示', {
    ...
    try {
      const res = await request.delete(`/outpatient-records/${row.recordId}`)
      if (res.code === 1) {
    ...
    if (valid) {
      try {
        const api = dialogType.value === 'add' ? 
          request.post('/outpatient-records', form.value) :
          request.put(`/outpatient-records/${form.value.recordId}`, {
