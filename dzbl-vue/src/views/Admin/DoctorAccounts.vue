...
const getList = async () => {
  ...
  try {
    ...
    const { data } = await request.get('/doctor-accounts', { params })
...
const submitAdd = async () => {
  ...
  try {
    await request.post('/doctor-accounts', addForm)
...
const submitEdit = async () => {
  ...
  submitLoading.value = true
  try {
    await request.put(`/doctor-accounts/${editForm.doctorId}/password`, {
...
const handleDelete = (row) => {
  ElMessageBox.confirm('确认删除该账号吗？', '提示', {
    ...
    try {
      await request.delete(`/doctor-accounts/${row.doctorId}`)
