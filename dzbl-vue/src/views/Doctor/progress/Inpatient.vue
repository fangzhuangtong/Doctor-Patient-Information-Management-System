<template>
  <div class="progress">
    <el-card class="progress-card">
      <template #header>
        <div class="card-header">
          <span>住院病程管理</span>
          <el-button type="primary" @click="handleAdd">新增病程</el-button>
        </div>
      </template>
      ...
      <el-table :data="tableData" style="width: 100%" v-loading="loading">
        <el-table-column prop="reportId" label="病程ID" width="100" />
        ...
        <el-table-column prop="content" label="病程内容" min-width="300" show-overflow-tooltip />
    ...
    <!-- 新增/编辑对话框 -->
    <el-dialog
      :title="dialogType === 'add' ? '新增病程' : '编辑病程'"
      v-model="dialogVisible"
      width="600px"
    >
      <el-form :model="form" :rules="rules" ref="formRef" label-width="100px">
        <el-form-item label="病历ID" prop="recordId" v-if="dialogType === 'add'">
          <el-input v-model.number="form.recordId" type="number" />
        </el-form-item>
        ...
        <el-form-item label="医生ID" prop="doctorId" v-if="dialogType === 'add'">
          <el-input v-model.number="form.doctorId" type="number" />
        </el-form-item>
        -->
        <el-form-item label="病程内容" prop="content">
          <el-input 
...
const getList = async () => {
  loading.value = true
  try {
    const res = await request.get('/medical-progresses', {
      params: {
        ...queryForm.value,
...
const rules = {
  ...
  content: [{ required: true, message: '请输入病程内容', trigger: 'blur' }]
}
...
const handleDelete = (row) => {
  ElMessageBox.confirm('确认删除该病程记录吗？', '提示', {
    type: 'warning'
  }).then(async () => {
    try {
      await request.delete(`/medical-progresses/${row.reportId}`)
    ...
    if (valid) {
      try {
        if (dialogType.value === 'add') {
          await request.post('/medical-progresses', form.value)
          ElMessage.success('添加成功')
        } else {
          const { reportId } = form.value
          await request.put(`/medical-progresses/${reportId}`, {
