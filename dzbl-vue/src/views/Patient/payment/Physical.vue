  ...
  <div class="payment">
    <el-card class="payment-card">
      ...
      <el-table :data="tableData" style="width: 100%" v-loading="loading">
        ...
        <el-table-column prop="relatedId" label="体检ID" width="100" />
...
const getList = async () => {
  ...
  try {
    ...
    if (res.code === 1) {
      // 过滤数据，只保留体检类型的订单
      const physicalOrders = res.data.list.filter(order => order.orderType === '体检')
      tableData.value = physicalOrders
      // 更新总数为过滤后的数量
      total.value = physicalOrders.length
    }
  } catch (error) {
