...
const getList = async () => {
  loading.value = true
  try {
    const res = await request.get('/inpatient-reports', {
      params: {
        ...queryForm.value,
...
const handleDelete = (row) => {
  ElMessageBox.confirm('确认删除该报告吗？', '提示', {
    ...
    try {
      await request.delete(`/inpatient-reports/${row.reportId}`)
    ...
    if (valid) {
      try {
        if (dialogType.value === 'add') {
          await request.post('/inpatient-reports', form.value)
          ElMessage.success('添加成功')
        } else {
          const { reportId } = form.value
          await request.put(`/inpatient-reports/${reportId}`, {
