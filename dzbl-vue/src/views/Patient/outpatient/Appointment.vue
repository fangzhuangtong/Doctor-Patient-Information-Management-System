        ...
        >
          ...
          <div class="doctor-info">
            ...
            <p class="schedule-info">
              <el-tag :type="schedule.scheduleType === '急诊' ? 'danger' : 'primary'" size="small">
                {{ schedule.scheduleType || '门诊' }}
              </el-tag>
        ...
        <el-table :data="scheduleData" style="width: 100%">
          ...
          <el-table-column label="出诊类型">
            <template #default="scope">
              <el-tag :type="scope.row.scheduleType === '急诊' ? 'danger' : 'primary'" size="small">
                {{ scope.row.scheduleType || '门诊' }}
              </el-tag>
            </template>
          </el-table-column>
...
const handleAddRegistration = (row) => {
  ...
  form.value = {
    departmentId: row.doctorInfo.departmentId,
    ...
    if (valid) {
      try {
        const res = await request.post('/registrations', {
          ...
          ...form.value,
          status: '待就诊',
          regTime: dayjs(form.value.regTime).format('YYYY-MM-DDTHH:mm:ss') // 格式化时间
        })
